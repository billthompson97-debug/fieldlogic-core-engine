...
function scoreJob(event: SavedOperationalEvent): AttentionJob {
  ...
  const nextAction =
    owner === 'service_department' && priority === 'urgent'
      ...
      : owner === 'service_department'
      ? 'Contact homeowner and review callback prevention steps'
...
export async function handleJobsNeedingAttentionRequest(db: D1Database): Promise<Response> {
  const rows = await db
    .prepare(
